...
class MonthlyFeeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('Backend.Student.Monthly_fee.index', $data);
    }
    ...
    public function StudentMonthlyFeeClasswise(Request $request)
    {
        ...
        foreach ($allStudent as $key => $value) {
            ...
            $monthlyFee = FeeAmount::where('fee_category_id', 3)->where('class_id', $value->class_id)->first();
            ...
            $html[$key]['tdsource'] .= '<a class="btn btn-sm btn-' . $color . '" title="Monthly Fee Slip" target="_blanks" href="' . route("monthly_fee.slip") . '?class_id=' . $value->class_id . '&student_id=' . $value->student_id . '&month=' . $request->month . '"><i class="fa fa-file-pdf-o"></i></a>';
            $html[$key]['tdsource'] .= '</td>';
        }
    ...
    // Generate Monthly Fee Slip 
    public function MonthlyFeeSlip(Request $request)
    {
        ...
        $pdf = PDF::loadView('Backend.Student.Monthly_fee.monthly_feeslip_pdf', $data);
